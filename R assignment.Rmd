...
Read data fang_et_al and SNP_position files
...
head(fang_et_al, 2, n=5)
head(SNPs, n=5)
Both files are structured as a list
typeof(fang_et_al)
typeof(SNPs)

Equally we have that both files are a data.frame according to the function class. For Genotype file we have all variables as factors, while for SNPs we have some variables as a integer.
...
levels(fang_et_al$Group)

### DATA PROCESSING

#SPLIT DATA INTO GROUPS
maize <- subset(fang_et_al, Group == "ZMMIL" | Group == "ZMMLR" | Group =="ZMMMR")

teosinte <- subset(fang_et_al, Group == "ZMPBA" | Group == "ZMPIL" | Group == "ZMPJA")

tripsacum <- subset(fang_et_al, !Group == "ZMPBA" & !Group == "ZMPIL" & !Group == "ZMPJA" & !Group == "ZMMIL" & !Group == "ZMMLR" & !Group=="ZMMMR") 
The results shows the total number of lines in the three files is the same as the totla number in the original file.
...
In the data.frame SNPs, the column 3 is for chromosome, the position of the chromosome is the column 4, while the column 1 (SNP_ID) is the common column with the transposed files. Then, we proceed to create a new file with only the information required from the snp_position.txt file:
snp_ID_chro_pos <- SNP_position[, c("SNP_ID", "Chromosome", "Position")]

Once we have the new data.frame, the data.frames are put into order of the transposed files as follows:
...
The files are sorted by snp_ID, therefore the SNPs and chromosome can be joined to it.
...
After the two uninformative columns are removed,each dataframe is sorted out by position in an ascending order. Different from UNIX here we do not need to have them in order to be able to extract the information. Sorting by position will produce a new file with the position sorted in an ascending order.
...
To do this, the data frame has to be ordered in a descending order and report that the values in the newd data fram are now characters in R.
